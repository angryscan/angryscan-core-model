...
class TextScanner(BaseScanner):
    ...
    def scan(self, path: str, timeout: int | None = None) -> ScanResult:
        ...
        try:
            ...
            # Check for embedded secrets in text content
            if file_size > 0 and file_size <= 10 * 1024 * 1024:  # up to 10MB
                try:
                    with open(path, encoding="utf-8", errors="replace") as f:
                        text_content = f.read()
                    self.check_for_embedded_secrets(text_content, result, path)
                except OSError:
                    pass  # Skip secrets check if read fails
