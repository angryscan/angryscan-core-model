...
class ManifestScanner(BaseScanner):
    ...
    def can_handle(cls, path: str) -> bool:
        ...
        # Accept any manifest extension (.json, .yaml, etc.) for secret/config checks, excluding web configs
        ext = os.path.splitext(path)[1].lower()
        return ext in MANIFEST_EXTENSIONS and filename not in web_configs
        ...
        try:
            ...
            if content:
                ...
                if isinstance(content, dict):
                    ...
                    # Check for embedded secrets in manifest/config values
                    self.check_for_embedded_secrets(content, result, path)
